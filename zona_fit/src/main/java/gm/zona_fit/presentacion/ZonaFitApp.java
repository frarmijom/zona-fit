package gm.zona_fit.presentacion;

import gm.zona_fit.datos.ClienteDAO;
import gm.zona_fit.datos.IClienteDAO;
    ...
    private static boolean ejecutarOpciones(Scanner consola, int opcion, IClienteDAO clienteDao) {
        ...
        switch (opcion){
            ...
            case 5 ->{
                ...
                // Creamos el objeto a modificar
                var cliente = new Cliente(idCliente);
                var eliminado = clienteDao.eliminarCiente(cliente);
                if (eliminado)
